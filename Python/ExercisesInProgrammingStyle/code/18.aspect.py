"""
이 형식은 기존 프로그램에서 지정된 위치 전후로 임의의 코드를 삽입하는
구체적인 목적을 위한 제한된 반영(restrained reflection) 으로 설명할 수 있다.

이렇게 하는 이유 중 하나는 소스코드에 대한 접근이나 변경은 원하지 않지만
해당 프로그램의 함수에 기능을 추가하고 싶은 경우나
일반적으로 프로그램 전체에 흩어져 있는 코드를 지역화해 개발을 단순화하는 것이다.

이 형식의 제약조건 중 하나는 영향을 받는 함수나 그 함수의 호출 위치를 편집하지 않고
부가 기능을 도입해야 한다는 것이다.

프로파일 할 함수 각각에 대해 기호표에 있는 그 이름의 결합 내용을
해당 함수를 감싼 함수로 대체한다.

예를 들면, extract_words라는 이름과 함수 몸체와의 결합을 끊고
extract_words라는 이름을 extract_words 함수를 매개변수로 취하는
profile 함수의 인스턴스와 결합시킨다.
"""
