...
```sh
yarn add -D typescript @babel/core @babel/preset-env @babel/preset-typescript @typescript-eslint/eslint-plugin @typescript-eslint/parser eslint prettier eslint-plugin-prettier
```
...
최신 자바스크립트 코드를 최대한 여러 브라우저에서 동작하도록 변경해주는 것
