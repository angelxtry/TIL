...
- 버그를 줄이는 설계
  - 버그를 방지하는 정의
    - 버그 중에서 가장 치명적이고 찾기 힘든 분류는 다양한 구성 요소의 개발자들이 나름대로 세운 가정이 서로 어긋날 때 생기는 시스템 버그들이다.
  - 명세 테스트
    - 코드가 한 줄이라도 만들어지기 전부터, 명세는 외부 테스팅 그룹으로 넘겨져서 완전성과 명확성을 검토받아야 한다.
  - 하향식 설계
    - 설계, 구현, 제품화의 각 과정은 하향식으로 이루어질 때 가장 좋은 결과를 가져온다.
    - 각 단계마다 가능한 한 추상화 수준이 높은 표기법을 써야한다.
    - 그럼으로써 더 세분화할 필요가 있을 때까지 개념은 드러내고 세부적인 사항은 숨길 수 있다.
    - 좋은 하향식 설계
      - 1 프로그램의 구조와 데이터 표현법이 명확하므로 각 모듈의 요구 사항과 기능을 엄밀하게 기술하기가 더 쉬워진다.
...
- 프로젝트에 엄청난 일정 지연이 발생했다는 얘기를 들으면, 아마도 큰 재난이 잇달아 닥쳐왔나 보다 상상하게 된다.
  - 하지만 재앙의 원인은 대개 토네이도라기보다 흰개미 때문일 경우가 많다.
- 마일스톤인가, 맷돌인가?
  - 마일스톤: 일정의 이벤트 목록에 기재된 각 항목
  - 구체적이고 명확하고 측정 가능한 이벤트여야 한다.
  - 상서에게 검증받기 쉬워야 한다.
  - 불분명한 마일스톤을 의욕을 갈아 으깨는 맷돌과도 같다.
  - 만성적인 일정 지연은 사기를 꺾는 주범이다.

- 야구 감독들은, 뛰어난 선수나 뛰어난 팀이라면 반드시 허슬이라는 비신체적 자질을 갖추고 있음을 알고 있다.
- 이 재능은 필요 이상으로 빨뤼 뛰고, 필요 이상으로 신속히 움직이며, 필요 이상으로 열심히 노력한다는 특성을 가지고 있다.
- 우리는 일정이 하루 지연되었다 해도 흥분해야만 한다.
...
- 책이 쓰였을 당시에도 옳았고 지금도 여전히 옳은 내용.
- 개념적 일관성과 아키텍트
  - 개념적 일관성
  - 아키텍트
    - 사용자가 인지하는 제품의 모든 측면에 개념적 일관성을 부여할 책임을 맡을 한 사람
    - 아키텍트는 감독, 팀 관리자는 제작자
  - 구현과 제품화로부터 아키텍처의 분리
  - 개념적 일관성은 제품 품질에 핵심적인 요소이고, 시스템 아키텍트를 두는 것은 개념적 일관성을 위한 가장 중요한 단계다.

- 피처 크립(feature creep): 제품에 기능이 덕지덕지 붙어가는 현상
  - 어떤 요구 사항이라도 그 자체로 "시장이 필요로 한다"라는 증거가 된다.
  - 원래의 시스템 아키텍트는 종종 더 좋은 곳으로 가버리고
  - 제품의 아키텍처는 사용자의 이해 전반을 균형 있게 반영해 본 경험이 적은 사람의 손에 놓이게 된다.

- 버리기 위해 만들지 말라: 폭포수 모델은 틀렸다.
  - "버리기 위한 계획을 세워라. 어쨌거나 버리게 될 것이다."는 너무 단순화되었다.
  - "버리기 위한 것을 만든다"는 사상에 담긴 가장 큰 오류는 폭포수 모델에 의해 순차적으로 만들어진다고 묵시적으로 가정했다는 점이다.

- 점진적 구축 모델이 더 낫다: 단계적인 세분화
  - 전체 과정을 담은 뼈대 시스템 만들기
  - 매일 밤 빌드하기
  - 점진적 구축과 고속 프로토타이핑

- 정보 은닉
  - 코드의 모듈이 잘 정의된 인터페이스로 캡슐화되어야 하며, 모듈 내부 구조는 담당한 프로그래머의 사유물로서 외부에 노출되지 않아야 한다.
  - 프로그래머들은 자기 것이 아닌 모듈의 내부 구조로부터 차단될 때 가장 효율적이 된다.
  - 295 page

- 사람이 전부다
  - 프로젝트 이관하기
    - 이관이 성공적이었던 경우는 한 번도 없다.
    - 훌륭한 문서화, 선진적인 설계, 원래 팀의 일부 인원이 남겨졌음에도 불구하고 새 팀은 개발을 사실상 처음부터 다시 시작했다.

- 권력을 포기할 수 있는 권력

## 부록 조금 오래된 컴퓨터 이야기

- 이 시절에는 메모리를 가리키는 단어로 코어를 흔히 썼고, 디버깅 등을 위해 메모리 내용을 출력하는 것을 코어 덤프(core dump)라고 불렀다.
- 비정상 종료나 의도적인 중지로 인한 메모리 덤프 파일은 여진히 core 라는 이름으로 떨어진다.

----

- 이후 대화식 텀퓨팅이 가능해지자, 입력 장치인 키보드와 출력 장치인 프린터를 합쳐 놓은 기계가 등장했다.
- 이것이 teletypewriter이며, 키보드로 명령을 입력하면 그 결과가 모니터 대신에 프린터로 출력되었다.
- 유닉스 계열에서 터미널을 뜻하는 TTY는 이 텔레타이프의 약자다.
