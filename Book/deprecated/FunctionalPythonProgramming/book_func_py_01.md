...
> 전체 무한 수열이 필요한 것은 아니다. 이 수열에서 어느 두 값의 차이가 충분히 작아서, 두 값을 모두 제곱근이라 불러도 문제가 없다면 수의 생성을 중단할 수 있다. 얼마나 가까운지를 나타내는 데 사용하는 기호는 그리스 문자인 입실론이다. 이를 우리가 감내할 수 있는 가장 큰 오차 범위라고 생각할 수 있다.
