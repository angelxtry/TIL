...
async def handler(client: socket.socket) -> None:
    while True:
        request: bytes = await async_recv(cluent, 100)
        if not request:
            client.close()
            return
        response: int = algorithm(int(request))
        await async_send(client, f'{respose}\n'.encode('ascii'))
...
def run_tasks() -> None:
    while any((TASKS, WAIT_READ, WAIT_WRITE)):
        while not TASKS:
            readables, writeables, _ = select.select(WAIT_READ, WAIT_WRITE, [])
        ...
        if action is Action.READ:
            ...
            raise RuntimeError(f'Unexcepted action {action!r})


if __name__ == '__main__':
    add_task(server(('loaclhost', 30303)))
    run_tasks()
