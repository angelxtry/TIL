...
> 일부 함수형 프로그래밍 언어는 우리가 만든 isprime()와 같은 단순한 재귀함수를 최적화할 수 있다. 최적화 컴파일러는 재귀호출을 부가 비용이 적게 드는 루프로 최적화해주낟 이러한 최적화를 수행하면 여러ㅓ 재귀호출 스택이 뭉개지기 때무에 최적화한 프로그램을 디버깅하기 힘들어진다 파이썬은 이러한 최적화를 수행하지 않는다. 단순함과 명확성을 위해 성능과 메모리를 희생시킨다.
