"""
주목할 만한 주요한 내용은 재편성 후 단어 세기를 병렬로 할 수 있다는 점인데,
단어 당 데이터를 재편성했기 때문이다.
그러므로 두 번째 맵 함수를 단어 세기에 적용할 수 있다.
이러한 재편성을 통해 파일 내 유일한 단어당 단어 세기 스레드/프로세서를
하나씩 사용할 수 있다.

어떤 자료 구조는 병렬로 처리하기가 쉽지 않지만
흔히 해당 데이터를 병렬화할 수 있게 하는 변형이 존재한다.
복잡한 데이터 집약적인 문제를 병렬화할 때는 재편성을 여러 번 거쳐야 할 수 있다.

데이터 센터 규모에서는 병렬화가 단순한 작어르 수행하는 서버에
데이터 구역을 보내는 것으로 끝난다.
